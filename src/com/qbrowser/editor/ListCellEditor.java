...
public class ListCellEditor extends DefaultCellEditor {
	...
	public Component getTableCellEditorComponent(JTable table, Object value, boolean isSelected, int row, int column) {
		...
		if(value instanceof JButton){
			...
			JTextArea tmp=new JTextArea(((JTextArea)value).getText());
			MouseListener[] ac2=((JTextArea)value).getMouseListeners();
			for(int i=0;i<ac2.length;i++){
				tmp.addMouseListener(ac2[i]);
			}
			comp=tmp;
			val=value;
			return comp;
		} else if(value instanceof JTextField){
			JTextField tmp=new JTextField(((JTextField)value).getText());
			ActionListener[] ac=((JTextField)value).getActionListeners();
			MouseListener[] ac2=((JTextField)value).getMouseListeners();
			for(int i=0;i<ac2.length;i++){
				tmp.addMouseListener(ac2[i]);
			}
			comp=tmp;
